...
const ResultDetail = () => {
    return (<>
        <div style={{ backgroundColor: resultData[4].color, padding: '80px 20px 20px 20px' }}>
            <h1 style={{ color: 'white' }}>{resultData[4].bugName}</h1>
            <h2 style={{ color: 'rgba(0,0,0,0.6)', marginBottom: '30px' }}>{resultData[4].alphabet}型のドリンク</h2>
            <div style={{ display: 'grid', placeContent: 'center' }}>
                <img src={resultData[4].img} style={{ marginBottom: '20px', width: '200px' }} />
            </div>
            <h1 style={{ color: 'white', marginBottom: '5px' }}>{resultData[4].drink}</h1>
            <p style={{ color: 'white', fontSize: '18px', fontWeight: 'bold' }}>{resultData[4].drinkMessage}</p>
        </div >
